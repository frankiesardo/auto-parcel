 ...
 * interface Foo ... { ... } } so that we can discover {@code @AutoParcel} classes that are nested
 * in an interface.
 *
...
final class AbstractMethodExtractor {
  ...
  // checking that the method has no parameters, because an @AutoParcel class will cause a compiler
  // error if there are abstract methods with parameters, since the @AutoParcel processor doesn't
